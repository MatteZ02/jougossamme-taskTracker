...
app.post("/api", (req, res) => {
    ...
    if (tasks.get(req.body.username.toLowerCase()).includes(req.body.taskID)) return res.sendStatus(400)
...
app.post("/api/login", (req, res) => {
    ...
    else res.status(200).json({ tasks: tasks.get(req.body.username.toLowerCase()) })
})

app.post("/api/role", (req, res) => {
    if (impostors.length > 0) {
        if (impostors.includes(req.body.username.toLowerCase())) res.json({ role: "impostor", others: impostors }).status(200)
...
app.get("/admin/api", (req, res) => res.json(Object.fromEntries(tasks)).status(200))
